...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        neighbour <- initial
        next <- newNeighborsOnly(neighborsWithHistory(neighbour._1, neighbour._2), explored)
      } yield next
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] =
    from(initial=Stream((startBlock, List())), explored=Set())
  ...
  lazy val solution: List[Move] =
    ...
    else pathsToGoal.minBy{ case (_, moves) => moves.length }.
      _2.reverse
}
